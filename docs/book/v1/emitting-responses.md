...
> [Solluzi/Solluzi-httphandlerrunner](https://docs.Solluzi.dev/Solluzi-httphandlerrunner).
...
interface, `Solluzi\Diactoros\Response\EmitterInterface`, defining a method `emit()` for emitting the
response.

Diactoros provides two implementations currently, both for working with
traditional Server API (SAPI) implementations: `Solluzi\Diactoros\Response\SapiEmitter`
and `Solluzi\Diactoros\Response\SapiStreamEmitter`.  Each uses the native `header()`
...
$emitter = new Solluzi\Diactoros\Response\SapiEmitter();
